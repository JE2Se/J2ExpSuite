...
dlist = []
#文件遍历
def ThinkphpScan(url):
    for file in os.listdir("./exphub/thinkphp/"):
        if os.path.splitext(file)[1] == '.py':
            if os.path.join(file) != "__init__.py" and os.path.join(file) != "ThinkphpScan.py":
    ...
    try:
        for defclass in dlist:
            ...
            exec("from exphub.thinkphp.{0} import {1}".format(defclass, defclass))
            defclass += "(url)"
            exec(defclass)
    except:
        logging.error("ThinkphpScan脚本出现异常")