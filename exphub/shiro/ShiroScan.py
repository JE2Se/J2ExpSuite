...
#文件遍历
def ShiroScan(url):
    for file in os.listdir("./exphub/shiro/"):
        if os.path.splitext(file)[1] == '.py':
            if os.path.join(file) != "__init__.py" and os.path.join(file) != "ShiroScan.py":
    ...
    try:
        for defclass in dlist:
            ...
            exec("from exphub.shiro.{0} import {1}".format(defclass, defclass))
            defclass += "(url)"
            exec(defclass)
    except:
        logging.error("ShiroScan脚本出现异常")