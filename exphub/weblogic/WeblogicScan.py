...
def WeblogicScan(url):
    for file in os.listdir("./exphub/weblogic/"):
        if os.path.splitext(file)[1] == '.py':
            if os.path.join(file) != "__init__.py" and os.path.join(file) != "WeblogicScan.py":
    ...
    try:
        for defclass in dlist:
            ...
            exec("from exphub.weblogic.{0} import {1}".format(defclass, defclass))
            defclass += "(url)"
            exec(defclass)
    except:
        logging.error("WeblogicScan脚本出现异常")